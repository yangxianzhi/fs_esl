...
CREATE TABLE `gateway` (
  `ID` varchar(36) NOT NULL,
  `caller_id_in_from` varchar(5) DEFAULT NULL COMMENT '通过该网关呼入时的callerID',
  `cid_type` varchar(10) DEFAULT NULL COMMENT '扩展的sip参数',
  `contact_params` varchar(32) DEFAULT NULL COMMENT '发送给联系人的扩展的sip参数',
  `expire_seconds` int(11) DEFAULT NULL COMMENT '超时时长，如果空白，则是3600秒',
  `extension` varchar(16) DEFAULT NULL COMMENT '呼入分机，可选项，如果为空，则与username相同',
  `extension_in_contact` varchar(5) DEFAULT NULL COMMENT 'Put the extension in the contact',
  `from_domain` varchar(32) DEFAULT NULL COMMENT '可选项,如果空白,与realm相同',
  `from_user` varchar(16) DEFAULT NULL COMMENT '可选项,如果空白,与username相同',
  `name` varchar(10) NOT NULL COMMENT '网关名称，不可以重复，必填项。',
  `password` varchar(32) NOT NULL COMMENT '账户密码，必填项',
  `ping` varchar(5) DEFAULT NULL COMMENT 'ping包的时间间隔',
  `proxy` varchar(32) DEFAULT NULL COMMENT '代理主机，可选项，如果空白，则与realm相同',
  `realm` varchar(32) NOT NULL COMMENT '认证域地址 必填项',
  `reg_id` int(11) DEFAULT NULL COMMENT '扩展的sip参数',
  `register` varchar(5) NOT NULL COMMENT '是否注册，必填项，填 ture or false ',
  `register_proxy` varchar(32) DEFAULT NULL COMMENT '注册代理，可选项，如果空白，则与proxy相同',
  `register_transport` varchar(4) DEFAULT NULL COMMENT '注册传输协议类型，默认UDP',
  `retry_seconds` int(11) DEFAULT NULL COMMENT '注册失败后，重试时间间隔。默认30秒',
  `rfc_5626` varchar(5) DEFAULT NULL COMMENT 'Abilitazione rfc5626',
  `username` varchar(16) NOT NULL COMMENT '帐号用户名 必填项',
  PRIMARY KEY (`ID`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8;
...
CREATE TABLE `sip_users` (
  ...
  `resonance` varchar(20) NOT NULL COMMENT '来电的振铃方式：0 只呼叫指定号码不共振也不顺振，1 共振手机和话机同振，2 顺振',
